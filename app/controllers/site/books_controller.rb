class Site::BooksController < ApplicationController
  ...
  def index
    ...
    @book = current_student.books.find(params[:book_id])
    @student_book = StudentBook.find_by(student_id: current_student.id, book_id: params[:book_id])
  ...
  def course
      ...
      filtered_levels = Level.where(course_id: @course.id)

      @student_books = current_student.books.where(level_id: filtered_levels)
  ...
  def save_page
    ...
    if student_book.current_page != page_number
      student_book.current_page = page_number
      student_book.save
    end
  end
